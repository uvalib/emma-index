...
@mock_dynamodb2
@mock.patch.object(metadata.config, "IA_COLLECTION_LIST", ['aceportal'])
@mock.patch.object(metadata.config, "IA_REPOSITORY_NAME", 'ace')
def test_transform(requests_mock):
    ...
    # metadata.config.IA_COLLECTION_LIST = ['aceportal']
    # metadata.config.IA_REPOSITORY_NAME = 'aceportal'
    ...
    for sample_record in transformed_records:
        ...
        assert sample_record['emma_repository'] == "ace"
...
@mock.patch.object(metadata.config, "IA_COLLECTION_LIST", ['aceportal'])
@mock.patch.object(metadata.config, "IA_FORMATS", [])
@mock_dynamodb2
def test_build_query():
    ...
    assert query == '_exists_:indexdate AND collection:(aceportal) AND mediatype:(texts)'
