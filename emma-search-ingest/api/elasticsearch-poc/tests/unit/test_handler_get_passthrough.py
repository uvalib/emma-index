...
import pytest
import re
from search_function import get_passthrough

def test_lambda_handler(apigw_event, requests_mock):
    # Set up the data
    elasticsearch_url_matcher = re.compile('.*_search.*')
    requests_mock.get(elasticsearch_url_matcher, text=sample_elasticsearch_response())

    # Run the test
    ret = get_passthrough.lambda_handler(apigw_event, "")

    # Check the results
    assert ret['statusCode'] == 200
    print(ret['body'])
    assert ret['body'] == sample_elasticsearch_response()
