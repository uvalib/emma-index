...
def get_commit_tag():
    ...
    if os.getenv('CODEBUILD_RESOLVED_SOURCE_VERSION'):
        ...
        print("Using CODEBUILD_RESOLVED_SOURCE_VERSION: %s" % [rev_hash])
    else:
        result = subprocess.run(['git', 'rev-parse', 'HEAD'], stdout=subprocess.PIPE)
        rev_hash = result.stdout.decode().replace('\n', '')
        print("Using git rev-parse HEAD: %s" % [rev_hash])
    return str(rev_hash)
...
def write_commit_details(commit_id, profile_param, s3_name):
    ...
    with open(detail_filename, "w") as outfile:
        subprocess.run(
            ['aws', 'codecommit', 'get-commit', '--repository-name', 'emma-maintenance-scripts', '--commit-id', commit_id],
            stdout=outfile)
